  ...
  public class Parser
  {
    ...
    Ast.FunctionCall ParseOopCall(ParseTreeNode node)
    {
      if (node.Term.Name == "OopCall")
      {
        ...
        call.Function = new Ast.TableAccess() { Expression = expr, Index = new Ast.StringLiteral() { Value = name,
    ...
    Ast.Assignment ParseFunctionDecl(ParseTreeNode node)
    {
      if (node.Term.Name == "FunctionDecl")
      {
        ...
        if (nameNode.ChildNodes.Count > 0)
        {
          ...
          expr = new Ast.TableAccess()
          {
            Expression = expr,
            Index = new Ast.StringLiteral() { Value = token.ValueString,
              lineNumber = token.Location.Line,
              columnNumber = token.Location.Column,
              filename = curParseTree.FileName }
          };
      ...
      if (node.Term != null)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            {
              Expression = prefix,
              Index = new Ast.StringLiteral() { Value = name },
              lineNumber = token.Location.Line,
              columnNumber = token.Location.Column,
              filename = curParseTree.FileName
            };
          ...
          while (true)
          {
            ...
            else
            {
              ...
              if (prefix.ChildNodes[0].Term.Name == "identifier")
              {
                ...
                key = new Ast.StringLiteral() { Value = token.ValueString,
                  lineNumber = token.Location.Line,
                  columnNumber = token.Location.Column,
                  filename = curParseTree.FileName
                  };
    ...
    Ast.IExpression ParseExpression(ParseTreeNode node)
    {
      if (node.Term.Name == "Expression")
      {
        ...
        {
          return new Ast.StringLiteral() { Value = (string)(token.Value),
            lineNumber = token.Location.Line,
            columnNumber = token.Location.Column,
            filename = curParseTree.FileName };
        }
        else if (token != null && token.Terminal is KeyTerm)
        {
          ...
          if (val == "true")
            return new Ast.BoolLiteral() { Value = true, lineNumber = token.Location.Line, columnNumber = token.Location.Column, filename = curParseTree.FileName };
          else if (val == "false")
            return new Ast.BoolLiteral() { Value = false, lineNumber = token.Location.Line, columnNumber = token.Location.Column, filename = curParseTree.FileName };
          else if (val == "nil")
            return new Ast.NilLiteral() { lineNumber = token.Location.Line, columnNumber = token.Location.Column, filename = curParseTree.FileName };
        }
          ...
          return new Ast.VarargsLiteral() { lineNumber = child.Span.Location.Line, columnNumber = child.Span.Location.Column, filename = curParseTree.FileName };
      }
